...
import { Plus, Trash2, Upload, X, ImageIcon, Eye, Calendar, MapPin, ShieldCheck } from "lucide-react"
...
interface EventFormWithPreviewProps {
  initialData?: {
    title?: string
    venue?: string
    city?: string
...
export function EventFormWithPreview({ initialData, eventId }: EventFormWithPreviewProps) {
  ...
  const validPrices = sections.map((s) => parseFloat(s.price || "0")).filter((p) => p > 0)
    ...
    <div className="flex gap-8 items-start">
      ...
      <form onSubmit={handleSubmit} className="flex-1 min-w-0 space-y-6">
        ...
        <Card title="Datas e horários">
          <div className="grid grid-cols-2 gap-4">
            <Field label="Início *">
              <input type="datetime-local" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} required className={inputClass} />
            </Field>
            <Field label="Término">
              <input type="datetime-local" value={dateTo} onChange={(e) => setDateTo(e.target.value)} className={inputClass} />
            </Field>
          </div>
          <Field label="Abertura de portões">
            <input type="datetime-local" value={gatesOpen} onChange={(e) => setGatesOpen(e.target.value)} className={inputClass} />
          </Field>
        </Card>
              ...
              {sections.map((section, i) => (
                <div key={i} className="flex items-start gap-3">
                  <div className="flex-1 grid grid-cols-3 gap-3">
                    <input placeholder="Nome do setor" value={section.name} onChange={(e) => updateSection(i, "name", e.target.value)} required className={inputClass} />
                    <input placeholder="Capacidade" type="number" min="1" value={section.capacity} onChange={(e) => updateSection(i, "capacity", e.target.value)} required className={inputClass} />
                    <input placeholder="Preço (R$)" type="number" min="0" step="0.01" value={section.price} onChange={(e) => updateSection(i, "price", e.target.value)} required className={inputClass} />
                  </div>
                  {sections.length > 1 && (
                    <button type="button" onClick={() => removeSection(i)} className="text-red-400 hover:text-red-600 mt-2 transition-colors">
                      <Trash2 className="w-4 h-4" />
                    </button>
                  )}
                </div>
              ))}
              <button type="button" onClick={addSection} className="flex items-center gap-1.5 text-sm text-[#2563EB] hover:text-blue-700 font-medium transition-colors">
        ...
        <Card title="Publicação">
          <label className="flex items-center gap-3 cursor-pointer">
            <input type="checkbox" checked={published} onChange={(e) => setPublished(e.target.checked)} className="w-4 h-4 rounded accent-[#2563EB]" />
            <span className="text-sm text-[#374151]">Publicar evento (visível ao público)</span>
          </label>
        </Card>

        <div className="flex items-center gap-3">
          <button type="submit" disabled={loading} className="bg-[#2563EB] hover:bg-blue-700 disabled:opacity-60 text-white font-semibold px-6 py-2.5 rounded-[8px] transition-colors">
            {loading ? "Salvando..." : isEdit ? "Salvar alterações" : "Criar evento"}
          </button>
          <button type="button" onClick={() => router.back()} className="border border-[#e5e7eb] bg-white hover:bg-[#f9fafb] text-[#374151] font-medium px-5 py-2.5 rounded-[8px] transition-colors">
            Cancelar
          </button>
        </div>
      </form>
      ...
      <div className="w-72 shrink-0 sticky top-6 space-y-4">
          ...
          <div className="px-3 py-3 flex items-center justify-between gap-2">
            <div className="flex flex-col gap-1 min-w-0">
              <div className="flex items-center gap-1 text-[#6b7280] text-xs">
                ...
                <span className="truncate">{dateFrom ? formatDateShort(new Date(dateFrom).toISOString()) : "Data"}</span>
              </div>
              <div className="flex items-center gap-1 text-[#6b7280] text-xs">
        ...
        <div className="flex items-center gap-2 text-sm font-semibold text-[#374151] mt-2">
          <Eye className="w-4 h-4 text-[#2563EB]" />
          Preview do card de compra
        </div>
        <div className="bg-white rounded-[8px] border border-[#e5e7eb] p-4 shadow-sm">
          <p className="text-xs text-[#6b7280] mb-1">A partir de</p>
          <p className="text-2xl font-bold text-[#2563EB] mb-3">
            {minPrice > 0 ? formatCurrency(minPrice) : "—"}
          </p>
          {sections.some((s) => s.name) && (
            <div className="space-y-1.5 mb-3">
              {sections.filter((s) => s.name).map((s, i) => (
                <div key={i} className="flex justify-between text-xs border border-[#e5e7eb] rounded-[4px] px-2.5 py-1.5">
                  <span className="text-[#374151]">{s.name}</span>
                  <span className="font-semibold text-[#111827]">
                    {s.price ? formatCurrency(parseFloat(s.price)) : "—"}
                  </span>
                </div>
              ))}
            </div>
          )}
          <div className="w-full bg-[#2563EB] text-white text-xs font-semibold text-center py-2 rounded-[6px]">
            Comprar Ingressos
          </div>
          <div className="flex items-center justify-center gap-1 mt-2 text-[10px] text-[#9ca3af]">
